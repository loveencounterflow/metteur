...
\\node[anchor=north west,xshift=❰xshift❱mm,yshift=❰yshift❱mm] at (current page.north west){%
  ...
  Template = (function() {
    ...
    //   \\node[anchor=north west,xshift=❰xshift❱mm,yshift=❰yshift❱mm] at (current page.north west){%
    //     \\fbox{\\includegraphics[width=❰page_width❱mm,height=❰page_height❱mm,units=-360,angle=❰angle❱,page=❰page_nr❱]{❰source_path❱}}};%
    //     \\end{tikzpicture}% ❰side❱ ❰column❱ p❰page_nr❱\n"""#.replace /\s*\n\s*/g, ''
  ...
  Metteur = class Metteur extends GUY.props.Strict_owner {
    ...
    _impose(cfg) {
      ...
      var Q, _column, _column_idx, _side, _slot_idx, _slot_map, column_width, doc_tpl, doc_tpl_path, format, i, j, k, l, len, len1, len2, len3, page_idx, page_nr, pdistro_idx, ref, ref1, ref2, ref3, ref4, ref5, row_height, sheet;
      ...
      while (true) {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          ...
          for (_column_idx = j = 0, len1 = ref1.length; j < len1; _column_idx = ++j) {
            ...
            ref4 = sheet[Q.column];
            for (page_idx = l = 0, len3 = ref4.length; l < len3; page_idx = ++l) {
              page_nr = ref4[page_idx];
              Q.page_nr = page_nr;
              pdistro_idx = (Q.sheet_nr - 1) * cfg.pps + page_nr - 1;
              Q.page_nr = (ref5 = cfg.pagedistro[pdistro_idx]) != null ? ref5 : -1/* NOTE: using -1 as error code */

...
\\node[anchor=north west,xshift=❰xshift❱mm,yshift=❰yshift❱mm] at (current page.north west){
  ...
  this.Template = (function() {
    class Template extends GUY.props.Strict_owner {
      ...
      //---------------------------------------------------------------------------------------------------------
      _compile() {
        var base, i, idx, is_mark, len, ref, segment_or_mark;
        is_mark = true;
        ...
        for (idx = i = 0, len = ref.length; i < len; idx = ++i) {
          ...
          if (is_mark = !is_mark) {
            this._segments.push([]);
            ((base = this._marks)[segment_or_mark] != null ? base[segment_or_mark] : base[segment_or_mark] = []).push(idx);
          } else {
      ...
      /* TAINT deprecate */      peek() {
        debug('^353-1^', this._segments);
        debug('^353-2^', this._segments.flat());

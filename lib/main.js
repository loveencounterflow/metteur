(function() {
  'use strict';
  var FS, GUY, Metteur, PATH, Template, debug, demo, echo, freeze, get, has, help, hide, info, isa, page_tpl, resolve, rpr, signature_tpl, types, urge, warn, whisper;
...
\\node[anchor=north west,xshift=❰xshift❱mm,yshift=❰yshift❱mm] at (current page.north west){%
  ...
  signature_tpl = `\\begin{tikzpicture}[overlay,remember picture]%
\\node[anchor=north west,xshift=+0mm,yshift=+30mm] at (current page.south west){%
  ...
  Metteur = class Metteur extends GUY.props.Strict_owner {
    ...
    _impose(cfg) {
      ...
      signature_tpl = new Template({
        template: signature_tpl,
        open: '❰',
        close: '❱',
        format
      });

(function() {
  'use strict';
  var FS, GUY, Metteur, PATH, Template, debug, demo, echo, freeze, get, has, help, hide, info, isa, layout, misfit, page_template, resolve, rpr, types, urge, warn, whisper;
...
\\node[anchor=north west,xshift=❰xshift❱mm,yshift=❰yshift❱mm] at (current page.north west){%
  ...
  Metteur = class Metteur extends GUY.props.Strict_owner {
    ...
    //---------------------------------------------------------------------------------------------------------
    xxx() {
      var Q, _column, _page_idx, _page_nr, _side, doc_template, doc_tpl, format, i, j, k, len, len1, len2, page_tpl, ref, ref1, ref2, sheet, source_path, template_path, tex_target_path;
      ...
      doc_template = FS.readFileSync(template_path, {
        encoding: 'utf-8'
      });
      format = function(x) {
        if (isa.text(x)) {
      ...
      Q.width = 297 / 4;
      Q.height = 210 / 2;
      Q.orientation = layout.orientation === 'ltr' ? +1 : -1;
      ref = ['recto', 'verso'];
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          ref2 = sheet[Q.column];
          for (_page_idx = k = 0, len2 = ref2.length; k < len2; _page_idx = ++k) {
            _page_nr = ref2[_page_idx];
            Q.page_nr = _page_nr;

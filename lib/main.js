(function() {
  'use strict';
  var FS, GUY, Metteur, PATH, debug, echo, freeze, help, hide, info, layout, page_template, resolve, rpr, types, urge, warn, whisper,
    splice = [].splice;
...
\\node[anchor=north west,xshift=❰xshift❱mm,yshift=❰yshift❱mm] at (current page.north west){
  ...
  this.Template = class Template extends GUY.props.Strict_owner {
    ...
    fill(cfg) {
      ...
      R = this.cfg.template;
      R = R.replace(this._cfg.rx, function(..._) {
        var key, ref, value;
        ref = _, [..._] = ref, [{key}] = splice.call(_, -1);
        // if key.startsWith '...'
        //   key = key[ 3 ... ]
        if ((value = cfg[key]) == null) {

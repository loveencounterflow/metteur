(function() {
  'use strict';
  var FS, GUY, Metteur, PATH, debug, echo, freeze, has, help, hide, info, layout, page_template, resolve, rpr, types, urge, warn, whisper,
    splice = [].splice;
...
\\node[anchor=north west,xshift=❰xshift❱mm,yshift=❰yshift❱mm] at (current page.north west){
  ...
  //===========================================================================================================
  this.Template = class Template extends GUY.props.Strict_owner {
    //---------------------------------------------------------------------------------------------------------
    constructor(cfg) {
      var close, open;
      super();
    ...
    _fill(mode, cfg) {
      ...
      R = R.replace(this._cfg.rx, ($0, ...args) => {
        ...
        if (!isa_text(v)) {
          throw new Error(`expected text, got a ${type_of(v)}`);
        }
        switch (dots) {
          case 'open':
            return `${v}${this.cfg.open}...${key}${this.cfg.close}`;
          default:
            return v;
        }
      });

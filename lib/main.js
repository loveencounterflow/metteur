(function() {
  'use strict';
  var FS, GUY, PATH, debug, echo, help, info, layout, page_template, resolve, rpr, types, urge, warn, whisper,
    splice = [].splice;
...
\\node[anchor=north west,xshift=❰xshift❱mm,yshift=❰yshift❱mm] at (current page.north west){
  ...
  this._escape_literal_for_regex = function(literal) {
    return literal.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  };

  //-----------------------------------------------------------------------------------------------------------
  this.interpolate = function(template, Q) {
    var R;
    R = template;
    R = R.replace(/❰(?<key>[^❱]*)❱/g, function(..._) {
      var key, ref, value;

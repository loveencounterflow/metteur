(function() {
  'use strict';
  var GUY, debug, help, id, info, list_of_page_nrs, types, urge, warn, whisper;
  ...
  //-----------------------------------------------------------------------------------------------------------
  types.declare.mtr_split(list_of_page_nrs = function(x) {
    var base, count, data, i, idx, len, pair, part, parts, pnr, pnrs;
    if (!this.isa.nonempty.text(x)) {
      return false;
    }
    data = ((base = this.state).data != null ? base.data : base.data = {}).mtr_split = {};
    parts = (function() {
    ...
    for (idx = i = 0, len = parts.length; i < len; idx = ++i) {
      ...
      if (Number.isNaN(pnr = parseInt(pnr))) {
        return false;
      }
      if (Number.isNaN(count = parseInt(count))) {
/* TAINT use @isa.nan when available */        return false;
      }
      /* TAINT use @isa.nan when available */      debug('^45-1^', {pnr, count});
      pnrs.push({pnr, count});
    }
    // pnrs = (  )
    data.pnrs = pnrs;

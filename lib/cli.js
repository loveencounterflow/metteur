(function() {
  'use strict';
  var $$, CP, FS, GUY, MIXA, Metteur, PATH, _run_tex, blue, debug, deep_copy, echo, fetch_pagecount, fetch_pagedistro, grey, help, info, isa, lime, path_from_executable_name, resolve, rpr, run_tex_etc, show_cfg, to_width, types, urge, validate, warn, whisper,
...
/* TAINT use loop, check *.aux for changes */
    for (count = i = 1; i <= 2; count = ++i) {
      try {
        ...
        error = error1;
        echo(FS.readFileSync(PATH.join(cfg.tex_working_path, 'xelatex-output'), {
          encoding: 'utf-8'
        }));
        warn(error.exitCode);
        warn(error.stderr);
